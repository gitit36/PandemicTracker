...
class StatsScraper:
    ...
    def checkVersion(
        self,
    ):  # Checks for last scraping time--scraping is only done if the current version is outdated
    ...
    def getTodaysIndex(self):
        startDate = date(
            2020, 4, 24
        )  # Arbitrary date (the day this parser was first written!), used to calculate the list index
        today = date.today()
        todaysIndex = (
            today - startDate
        ).days + 97  # This will give the index used for the most recent entry the BeautifulSoup scrapes
    ...
    def scrapeCases(self):  # Scrapes the most recent case data
        ...
        if self.checkVersion() == False:
            return (
                "Stats database already up to date.",
                self.globalConfirmed,
                self.globalDeaths,
                self.globalRecovered,
            )

        # Accessing the github pages
        responseConfirmed = requests.get(
            "https://github.com/CSSEGISandData/COVID-19/blob/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv"
        )
        responseDeaths = requests.get(
            "https://github.com/CSSEGISandData/COVID-19/blob/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"
        )
        responseRecovered = requests.get(
            "https://github.com/CSSEGISandData/COVID-19/blob/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv"
        )

        if (
            responseConfirmed.status_code != 200
            or responseDeaths.status_code != 200
            or responseRecovered.status_code != 200
        ):
        ...
        for i in range(2, 266):  # There are 266 rows in the dataset
            ...
            countryName = countryConfirmed.find_all("td")[
                2
            ].get_text()  # Getting the name
